...
#define GLogError(format, ...)      DDLogError((@"[函数名:%s]" "[行号:%d]" format), __FUNCTION__, __LINE__, ##__VA_ARGS__);
#define GLogWarn(format, ...)       DDLogWarn((@"[函数名:%s]" "[行号:%d]" format),__FUNCTION__, __LINE__, ##__VA_ARGS__);
#define GLogInfo(format, ...)       DDLogInfo((@"[函数名:%s]" "[行号:%d]" format),__FUNCTION__, __LINE__, ##__VA_ARGS__);
#define GLogDebug(format, ...)      DDLogDebug((@"[函数名:%s]" "[行号:%d]" format),__FUNCTION__, __LINE__, ##__VA_ARGS__);
#define GLogVerbose(format, ...)    DDLogVerbose((@"[函数名:%s]" "[行号:%d]" format),__FUNCTION__, __LINE__, ##__VA_ARGS__);

/*
    1,GLogVerbose:前期开发阶段使用此打印，主要用于基本的数据调试，基本的日志布局，由于基本所以繁琐，在后期调试bug时，谨慎使用。
    2,GLogDebug:用来开发后期调试bug使用，关键bug疑似位置用此打印。业务逻辑的处理跳转。
