...
///设置导航控制器的右边样式字体颜色（默认白色），文字内容（默认为空）
- (void)setForNavWithRightTextColor:(UIColor *)backTextColor
                           BackText:(NSString *)backText;
{
    UIBarButtonItem *rightBarItemTitle = nil;
    if (backText != nil) {
        rightBarItemTitle = [[UIBarButtonItem alloc] initWithTitle:backText
                                                            style:UIBarButtonItemStylePlain
                                                           target:self
                                                           action:@selector(clickRight)];
    }else{
        rightBarItemTitle = [[UIBarButtonItem alloc] initWithTitle:@""
                                                            style:UIBarButtonItemStylePlain
                                                            target:self
                                                            action:@selector(clickRight)];
    }
    if (backTextColor != nil) {
        rightBarItemTitle.tintColor = backTextColor;
    }else{
        rightBarItemTitle.tintColor = [UIColor whiteColor];
    }
    
    self.navigationItem.rightBarButtonItem = rightBarItemTitle;
}
- (void)clickRight
{
    if (self.callbackForRightItem) {
        self.callbackForRightItem();
    }
}
@end
