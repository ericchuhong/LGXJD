...
///创建对象到父视图（必填），确定文字大小（必填）.文字颜色（默认黑色）.文字内容（默认按钮）.背景颜色（默认蓝色）.字体名称（默认宏定义）.圆角
...
{
    UIButton *button = [UIButton initButtonWithFatherView:fatherView Size:size TitleColor:color backgroundColor:bgcolor Title:title FontName:fontName];
    ...
    if (GScreenWidth == 375)
    {
        button.layer.cornerRadius = cornerRadius + 3;
    }
    if (GScreenWidth == 320)
    {
...
+ (UIButton *)initButtonWithFatherView:(UIView *)fatherView
                       ...
                       backgroundColor:(UIColor *)bgcolor
                              ...
                              FontName:(NSString *)fontName;
{
    UIButton *button = [UIButton initButtonWithFatherView:fatherView Size:size TitleColor:color backgroundColor:bgcolor Title:title];
